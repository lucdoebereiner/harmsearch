...
main =
  let db =
        [ shuffled 196 $
          only (\h -> not (H.isArtificial h) && not (H.isHalfHarm h)) $
          withStrings [2, 3, 4] $
                 ...
                 H.psetPredPairIdx
                   (\i p1 p2 ->
                      let x = (fromIntegral i) / (fromIntegral n)
                          thisOverlap = lineStartEnd x 0.2 0.2 0.2 0.2
                          thisThresh = lineStartEnd x 1.6 1.6 0.2 0.2
                       in (H.scoreOverlap p1 p2) >= thisOverlap &&
                          (H.scoreHarms p1 p2) >= thisThresh)
                   fingeringPairs &&
                 H.psetPred
                   (\p1 p2 ->
                      (H.scoreHarms p1 p2) >= thresh &&
                      (H.scoreOverlap p1 p2) >= overlap)
                   vcs &&
                 H.psetPred
                   (\p1 p2 ->
                      (H.scoreHarms p1 p2) >= thresh &&
   ...
   in do let solutions = S.collectSolutions 1 solver
         ...
         let stavesVc =
               D.harmonicsToStaves
                 ...
                 (Just (integrate 0 [6, 10, 4, 14, 9, 3, 8, 12, 16, 5, 11, 7]))
                 vc
         let stavesDb =
               D.harmonicsToStaves
                 D.F8Clef
                 ...
                 (Just (integrate 0 [10, 4, 14, 9, 3, 8, 12, 16, 5, 11, 7, 6]))
                 db
